/**
 * @file        kl720_demo_generic_inference_crop.c
...
static char _image_file_path[128] = "../../res/images/people_cars_street_1500x1500.bmp";
static int _loop = 50;
...
static kp_generic_raw_result_header_t _output_desc_0;
static kp_generic_raw_result_header_t _output_desc_1;
...
int main(int argc, char *argv[])
{
    ...
    _input_desc.crop_count = 2;                        // number of crop area, 0 means no cropping
    _input_desc.inf_crop[0].crop_number = 0;           // crop number for sync, could be specified by user
    _input_desc.inf_crop[0].x1 = 860;
    _input_desc.inf_crop[0].y1 = 618;
    _input_desc.inf_crop[0].width = 630;
    _input_desc.inf_crop[0].height = 630;
    _input_desc.inf_crop[1].crop_number = 1;           // crop number for sync, could be specified by user
    _input_desc.inf_crop[1].x1 = 98;
    _input_desc.inf_crop[1].y1 = 583;
    _input_desc.inf_crop[1].width = 600;
    _input_desc.inf_crop[1].height = 600;
    ...
    for (int i = 0; i < _loop; i++)
    {
        ...
        ret = kp_generic_raw_inference_receive(_device, &_output_desc_0, raw_output_buf_crop_box_0, raw_buf_size);
        if (ret != KP_SUCCESS)
            break;
        ...
        ret = kp_generic_raw_inference_receive(_device, &_output_desc_1, raw_output_buf_crop_box_1, raw_buf_size);
        if (ret != KP_SUCCESS)
            break;
    ...
    output_nodes[0] = kp_generic_inference_retrieve_float_node(0, raw_output_buf_crop_box_0, KP_CHANNEL_ORDERING_CHW);
    output_nodes[1] = kp_generic_inference_retrieve_float_node(1, raw_output_buf_crop_box_0, KP_CHANNEL_ORDERING_CHW);
    output_nodes[2] = kp_generic_inference_retrieve_float_node(2, raw_output_buf_crop_box_0, KP_CHANNEL_ORDERING_CHW);

    // post-process yolo v5 output nodes to class/bounding boxes
    post_process_yolo_v5_720(output_nodes, _output_desc_0.num_output_node, &_output_desc_0.pre_proc_info, 0.15, yolo_result);

    helper_print_yolo_box_of_crop_area_on_bmp(yolo_result, _image_file_path, _input_desc.inf_crop[0]);

    // retrieve output nodes in floating point format in second crop box
    output_nodes[0] = kp_generic_inference_retrieve_float_node(0, raw_output_buf_crop_box_1, KP_CHANNEL_ORDERING_CHW);
    output_nodes[1] = kp_generic_inference_retrieve_float_node(1, raw_output_buf_crop_box_1, KP_CHANNEL_ORDERING_CHW);
    output_nodes[2] = kp_generic_inference_retrieve_float_node(2, raw_output_buf_crop_box_1, KP_CHANNEL_ORDERING_CHW);

    // post-process yolo v5 output nodes to class/bounding boxes
    post_process_yolo_v5_720(output_nodes, _output_desc_1.num_output_node, &_output_desc_1.pre_proc_info, 0.15, yolo_result);

    helper_print_yolo_box_of_crop_area_on_bmp(yolo_result, _image_file_path, _input_desc.inf_crop[1]);
