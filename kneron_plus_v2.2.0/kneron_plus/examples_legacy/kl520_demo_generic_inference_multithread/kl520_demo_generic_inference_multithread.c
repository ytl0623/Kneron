...
void *image_send_function(void *data)
{
    ...
    {
        int ret = kp_generic_raw_inference_send(_device, &_input_desc, (uint8_t *)_img_buf);
        if (ret != KP_SUCCESS)
        {
...
void *result_receive_function(void *data)
{
    ...
    for (int i = 0; i < _loop; i++)
    {
        int ret = kp_generic_raw_inference_receive(_device, &_output_desc, _raw_output_buf, _raw_buf_size);
        if (ret != KP_SUCCESS)
        {
...
int main(int argc, char *argv[])
{
    ...
    {
        printf("[error] device is not run at high speed.\n");
        return -1;
    }
    ...
    output_nodes[0] = kp_generic_inference_retrieve_float_node(0, _raw_output_buf, KP_CHANNEL_ORDERING_HCW);
    output_nodes[1] = kp_generic_inference_retrieve_float_node(1, _raw_output_buf, KP_CHANNEL_ORDERING_HCW);
