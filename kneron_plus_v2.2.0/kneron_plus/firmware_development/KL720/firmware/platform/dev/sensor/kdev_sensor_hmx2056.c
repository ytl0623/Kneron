...
#if (V2K_ENABLE_TYPE == V2K_ENABLE_HMX2056) || (V2K_ENABLE_TYPE == V2K_ENABLE_HMX2056_OV9286)
#include <stdlib.h>
#include "base.h"
...
static int hmx2056_write_reg(struct sensor_device *sensor_dev, uint16_t reg, uint8_t data)
{
    int ret;
...
void hmx2056_init(struct sensor_device *sensor_dev, struct sensor_init_seq *seq)
{
    ...
    for (init_fnc_ptr = seq; ; ++init_fnc_ptr) {
        //DSG("dev->adapter=%p dev->addr=%x init_fnc_ptr->addr=%x init_fnc_ptr->value=%x",
        //dev->adapter, dev->addr, init_fnc_ptr->addr, init_fnc_ptr->value);
        if(init_fnc_ptr->addr == 0 && init_fnc_ptr->value == 0) break; //reaches end
        hmx2056_write_reg(dev, init_fnc_ptr->addr , (uint8_t)(init_fnc_ptr->value & 0xFF));
    }
}
...
static const struct sensor_win_size *hmx2056_select_win(uint32_t *width, uint32_t *height)
{
...
static
int hmx2056_set_params(struct sensor_device *sensor_dev)
{
