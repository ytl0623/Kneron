...
*  Description:
*  ------------
*  This SPI Flash driver is specific for Mxic SPI Flash Access
*  HW: Faraday FTSPI020
*
******************************************************************************/
...
//#define FLASH_MXIC_DBG
#ifdef FLASH_MXIC_DBG
#define flash_msg(fmt, ...) info_msg("[MXIC_FLASH] " fmt, ##__VA_ARGS__)
#else
...
void kdev_flash_read_status(void)
{
    ...
    RDCR  &= ~0x08; /* disable Top/Bottom protect */
    RDCR  |= 0x07; /* driver output strength 111 30Ohms(25L512) / 15Ohms(25L256) */
...
{
    uint16_t i=0;
    uint32_t nstart_index = start_addr / st_flash_info.sector_size_Bytes;
    uint32_t nend_index = end_addr/st_flash_info.sector_size_Bytes;
