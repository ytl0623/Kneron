...
/**
  Get +LINK_CONN response parameters (see +SYSMSG).
...
/**
  ...
  Format S: AT+UART=<baudrate>,<databits>,<stopbits>,<parity>,<flow control>
  Format Q: AT+UART?

  Example S: AT+UART=115200,8,1,0,0\r\n
...
int32_t AT_Cmd_ConfigUART (uint32_t at_cmode, uint32_t baudrate, uint32_t databits, uint32_t stop_par_flowc) {
  ...
  /* Open AT command (AT+<cmd><mode> */
  n = CmdOpen (CMD_UART, AT_CMODE_SET, out);
...
/**
  ...
  Response Q: +UART:<baudrate>,<databits>,<stopbits>,<parity>,<flow control>\r\n\r\n\OK
...
int32_t AT_Cmd_SysMessages (uint32_t n) {
  ...
  /* Open AT command (AT+<cmd><mode> */
  k = CmdOpen (CMD_SYSMSG, AT_CMODE_SET, out);

  /* Add command arguments */
  k += sprintf (&out[k], "%d", n);

  /* Append CRLF and send command */
  return (CmdSend(CMD_SYSMSG, out, k));
}
...
int32_t AT_Cmd_CurrentMode (uint32_t at_cmode, uint32_t mode) {
  ...
  /* Open AT command (AT+<cmd><mode> */
  n = CmdOpen (CMD_CWMODE, at_cmode, out);
  ...
  /* Append CRLF and send command */
  return (CmdSend(CMD_CWMODE, out, n));
}

/**
  Get response to CurrentMode command

  Response Q: +CWMODE:<mode>
  Example  Q: +CWMODE:3\r\n\r\n\OK
...
/**
  ...
  Format S: AT+CWJAP=<ssid>,<pwd>[,<bssid>]
  Format Q: AT+CWJAP?
...
int32_t AT_Cmd_ConnectAP (uint32_t at_cmode, const char *ssid, const char *pwd, const char *bssid) {
  ...
  /* Open AT command (AT+<cmd><mode> */
  n = CmdOpen (CMD_CWJAP, at_cmode, out);
  ...
  /* Append CRLF and send command */
  return (CmdSend(CMD_CWJAP, out, n));
}

/**
  Response to ESP_ConnectAP command.

  Response Q: +CWJAP:<ssid>,<bssid>,<channel>,<rssi>
  Example  Q: +CWJAP:"AP_SSID","xx:xx:xx:xx:xx:xx",6,-60
...
/**
  ...
  Format: AT+CWSAP=<ssid>,<pwd>,<chl>,<ecn>[,<max conn>][,<ssid hidden>]
*/
int32_t AT_Cmd_ConfigureAP (uint32_t at_cmode, AT_DATA_CWSAP *cfg) {
  ...
  /* Open AT command (AT+<cmd><mode> */
  n = CmdOpen (CMD_CWSAP, at_cmode, out);
  ...
  /* Append CRLF and send command */
  return (CmdSend(CMD_CWSAP, out, n));
}
...
int32_t AT_Cmd_StationMAC (uint32_t at_cmode, const uint8_t mac[]) {
  ...
  /* Open AT command (AT+<cmd><mode> */
  n = CmdOpen (CMD_CIPSTAMAC, at_cmode, out);
  ...
  /* Append CRLF and send command */
  return (CmdSend(CMD_CIPSTAMAC, out, n));
}

/**
  Get response to StationMAC command

  String form: +CIPSTAMAC:<mac>\r\n\r\nOK
...
int32_t AT_Cmd_AccessPointMAC (uint32_t at_cmode, uint8_t mac[]) {
  ...
  /* Open AT command (AT+<cmd><mode> */
  n = CmdOpen (CMD_CIPAPMAC, at_cmode, out);
  ...
  /* Append CRLF and send command */
  return (CmdSend(CMD_CIPAPMAC, out, n));
}
...
/**
  ...
  Format S: AT+CIPSTA=<ip>,<gateway>,<netmask>
  Format Q: AT+CIPSTA?

  Example: AT+CIPSTA="192.168.1.100","192.168.6.1","255.255.255.0"
...
int32_t AT_Cmd_StationIP (uint32_t at_cmode, uint8_t ip[], uint8_t gw[], uint8_t mask[]) {
  ...
  /* Open AT command (AT+<cmd><mode> */
  n = CmdOpen (CMD_CIPSTA, at_cmode, out);
  ...
  /* Append CRLF and send command */
  return (CmdSend(CMD_CIPSTA, out, n));
}
...
/**
  ...
  Example:  +CIPSTA:ip:"192.168.1.155"
...
int32_t AT_Cmd_AccessPointIP (uint32_t at_cmode, uint8_t ip[], uint8_t gw[], uint8_t mask[]) {
  ...
  /* Open AT command (AT+<cmd><mode> */
  n = CmdOpen (CMD_CIPAP, at_cmode, out);
  ...
  /* Append CRLF and send command */
  return (CmdSend(CMD_CIPAP, out, n));
}
...
/**
  ...
  Format S: AT+CIPDNS=<enable>[,<DNS server0>,<DNS server1>]
  Format S: AT+CIPDNS?

  Example: AT+CIPDNS=1,"192.168.0.1"
...
int32_t AT_Cmd_DNS (uint32_t at_cmode, uint32_t enable, uint8_t dns0[], uint8_t dns1[]) {
  ...
  /* Open AT command (AT+<cmd><mode> */
  n = CmdOpen (CMD_CIPDNS, at_cmode, out);
  ...
  /* Append CRLF and send command */
  return (CmdSend(CMD_CIPDNS, out, n));
}
...
int32_t AT_Cmd_DHCP (uint32_t at_cmode, uint32_t operate, uint32_t mode) {
  ...
  /* Open AT command (AT+<cmd><mode> */
  n = CmdOpen (CMD_CWDHCP, at_cmode, out);
  ...
  /* Append CRLF and send command */
  return (CmdSend(CMD_CWDHCP, out, n));
}
#else
/**
  Set/Query DHCP state

  Format S: AT+CWDHCP=<mode>,<en>
  Format Q: AT+CWDHCP?
...
/**
  ...
  Format S: AT+CWDHCPS=<enable>,<lease time>,<start IP>,<end IP>
  Format Q: AT+CWDHCPS?

  Example: AT+CWDHCPS=1,3,"192.168.4.10","192.168.4.15"
...
int32_t AT_Cmd_RangeDHCP (uint32_t at_cmode, uint32_t en_tlease, uint8_t ip_start[], uint8_t ip_end[]) {
  ...
  /* Open AT command (AT+<cmd><mode> */
  n = CmdOpen (CMD_CWDHCPS, at_cmode, out);
  ...
  /* Append CRLF and send command */
  return (CmdSend(CMD_CWDHCPS, out, n));
}

/**
  Get response to RangeDHCP command

  Response Q: +CWDHCPS=<lease time>,<start IP>,<end IP>
  Example  Q: +CWDHCPS:???
...
/**
  Set SSL config .
	...
	Format S: AT+CIPSSLCCONF=<size>, <size> :[2049,4096]

  Example S: AT+CIPSSLCCONF=2 //load CA to certificate server

  Generic response is expected.

  \param[in]  at_cmode    Command mode (inquiry, set, exec)
	\param[in]  mode    BIT0:ssl cert and private key  BIT1:load CA
  \return execution status:
          0: OK, -1 on error
	\note Supports the establishment of up to 1 SSL connection
*/
int32_t AT_Cmd_SetSSLConf(uint32_t at_cmode,uint16_t mode)
{
	char out[64] = {0};
	int32_t n ;
	
	/* Open AT command (AT+<cmd><mode> */
  n = CmdOpen (CMD_CIPSSLCCONF, at_cmode, out);
	...
	return (CmdSend(CMD_CIPSSLCCONF, out, n));
...
/**
  Set SSL buffer size.
	...
	Format S: AT+CIPSSLSIZE=<size>, <size> :[2049,4096]

  Example S: AT+CIPSSLSIZE=2048

  Generic response is expected.

  \param[in]  at_cmode    Command mode (inquiry, set, exec)
  \param[in]  buf_size    SSL Buffer size ,[2048,4096]  
  \return execution status:
          0: OK, -1 on error
	\note Supports the establishment of up to 1 SSL connection
*/
int32_t AT_Cmd_SetSSLSize(uint32_t at_cmode,uint16_t buf_size)
{
	char out[64] = {0};
	int32_t n ;
	
	/* Open AT command (AT+<cmd><mode> */
  n = CmdOpen (CMD_CIPSSLSIZE, at_cmode, out);
	
	if (at_cmode == AT_CMODE_SET) {
    /* Add command arguments */
    n += sprintf (&out[n], "%d",buf_size);
	}

	return (CmdSend(CMD_CIPSSLSIZE, out, n));
