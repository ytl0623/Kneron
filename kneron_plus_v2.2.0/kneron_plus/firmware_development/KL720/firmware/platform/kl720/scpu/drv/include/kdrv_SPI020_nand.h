...
#define FLASH_4BYTES_CMD_EN     0x00 //NAND flash doesn't support 4byte address cmd
...
typedef struct {
    uint32_t address_length     : 3;    /* Default Value 0x3 */
                                         ...
                                         * Others: Reserved 
                                         * For the XIP port access, the value is decided by the SPI_addr_cyc port.
                                         * When the address length is 0, there will be no dummy cycle ([23:16]) and continuous read mode code (0x000C[23 :16]) will occur. */
...
typedef struct {
    uint32_t Data_counter       : 32;   /* Read/Write data counter */ /* Default Value 0 */
                                        /* This register must be set to ．0・ when performing the read status command.
                                         ...
                                         * this register is not allowed to be filled as ．0・.
                                         * However,  such as ：read status； or ：write enable； instruction,
                                         * this register must set to ．0・. */
} kdrv_spif_cmd2_t;
...
typedef struct {
    ...
    uint32_t write_enable               : 1;    /* Default Value 0 */
                                                 ...
                                                 * users must set the write enable = ．1・ for other SPI commands.
                                                 * Please note that in the write data or erase Flash command, write enable is set to ．1・.
                                                 * Only when in the read data or read status command, write enable must be set to ．0・. */
    uint32_t read_status_en             : 1;    /* Enable the Read SPI status */ /* Default Value 0 */
                                                /* It is available at write_enable = ．0・ and users must issue the SPI read status command.
                                                 * 1'b0: Disable.
                                                 * 1・b1: Enable. */
    ...
    uint32_t DTR_mode                   : 1;    /* DTR (Double Transfer Rate) mode */ /* Default Value 0 */
                                                /* 1'b0: Disable
                                                 * The read status, write data command, and DTR mode must be set to ．0・.
                                                 * 1'b1: Enable
                                                 * The DTR mode is valid at the read data command.
                                                 * The DTR mode will be invalid when hardware configuration is ：FTSPI020_DTR_MODE_OFF；.
    ...
    uint32_t continuous_read_op_code    : 8;    /* Continuous read mode operate code */ /* Default Value HwCfg */
                                                 ...
                                                 * default value will depend on FTSPI020_XIP_OPCODE; otherwise, it is ．0・. */
    uint32_t instruction_code           : 8;    /* Instruction code */ /* Default Value HwCfg */
                                                 ...
                                                 * depend on FTSPI020_XIP_INSTRUCTION; otherwise, it is ．0・. */
} kdrv_spif_cmd3_t;
...
typedef struct {
    ...
    uint32_t trace_delay      : 4;  /* Default Value 0x3 */
                                    /* Trace delay for turnaround stage
                                    * 4・b0000 ~ 4・b1111: 1 cycle ~ 16 cycles */
 ...
 ********************************************/
#define ADR_STATUS_REG_1          0xA0
#define ADR_STATUS_REG_2          0xB0
#define ADR_STATUS_REG_3          0xC0
#define ADR_STATUS_REG_4          0xD0
#define ADR_STATUS_REG_5          0xF0
...
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 0, dum_1st_cyc = 0, address length = 0 */
#define SPI020_9F_CMD1      0x01000001
...
/*******************************************
 * for read chip id command (0x9F) GD F-series
 ********************************************/
/* Set command word 0, set SPI flash address to 0 */
#define SPI020_9F_CMD0_f      0x0
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 0, dum_1st_cyc = 0, address length = 0 */
#define SPI020_9F_CMD1_f      0x01000000
/* Set command word 2, set data count to 3 */
#define SPI020_9F_CMD2_f      0x3
/* Set command word 4, instrction code = 0x9F, contiune read = 0,
   start_ce = ??, spi mode = 0, DTR mode = 0, status = 0,
   status_en = 0, write enable = 0, intr_en = ? */
#define SPI020_9F_CMD3_f      (0x9F000000|SPI020_CE_VALUE|SPI020_INTR_CFG)
/*******************************************
 ...
 ********************************************/
/* Set command word 0, set SPI flash address to 0 */
#define SPI020_05_CMD0      ADR_STATUS_REG_3
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 0, dum_1st_cyc = 0, address length = 0 */
#define SPI020_05_CMD1      0x01000001
/* Set command word 2, set data count to 0 */
#define SPI020_05_CMD2      0x0
/* Set command word 4, instrction code = 0x05, contiune read = 0,
   start_ce = ??, spi mode = 0, DTR mode = 0, status = 0,
   status_en = 1, write enable = 0, intr_en = ? */
#define SPI020_05_CMD3      (0x0F000004|SPI020_CE_VALUE|SPI020_INTR_CFG)
/*******************************************
 * for status command (0x05)
 ********************************************/
/* Set command word 0, set SPI flash address to 0 */
#define SPI020_05_CMD0_w      ADR_STATUS_REG_3
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 0, dum_1st_cyc = 0, address length = 0 */
#define SPI020_05_CMD1_w      0x01000001
/* Set command word 2, set data count to 0 */
#define SPI020_05_CMD2_w      0x1
/* Set command word 4, instrction code = 0x05, contiune read = 0,
   start_ce = ??, spi mode = 0, DTR mode = 0, status = 0,
   status_en = 1, write enable = 0, intr_en = ? */
#define SPI020_05_CMD3_w      (0x0F000008|SPI020_CE_VALUE|SPI020_INTR_CFG)
/*******************************************
...
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 0, dum_1st_cyc = 0, address length = 3 */
#define SPI020_03_CMD1      0x01080002 /* F 0x01000003 */ /* E 0x01080002 */
/* Set command word 2, set data count by input parameter */
 ...
 ********************************************/
/* Set command word 0, set SPI flash address by input parameter */
#define SPI020_03_CMD0_f      0x0
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 0, dum_1st_cyc = 0, address length = 3 */
#define SPI020_03_CMD1_f      0x01000003 /* F 0x01000003 */ /* E 0x01080002 */
/* Set command word 2, set data count by input parameter */
#define SPI020_03_CMD2_f      0x0
/* Set command word 4, instrction code = 0x03, contiune read = 0,
   start_ce = ??, spi mode = 0, DTR mode = 0, status = 0,
   status_en = 0, write enable = 0, intr_en = ? */
#define SPI020_03_CMD3_f      (0x03000000|SPI020_CE_VALUE|SPI020_INTR_CFG)
/*******************************************
 * for Page Data Read command (0x13)
 ********************************************/
/* Set command word 0, set page address by input parameter */
#define SPI020_13_CMD0      0x00FF0102
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 0, dum_1st_cyc = 0, address length = 4 */
#define SPI020_13_CMD1      0x01000003
/* Set command word 2, set data count by input parameter */
#define SPI020_13_CMD2      0x0
/* Set command word 4, instrction code = 0x13, contiune read = 0,
   start_ce = ??, spi mode = 0, DTR mode = 0, status = 0,
   status_en = 0, write enable = 0, intr_en = ? */
#define SPI020_13_CMD3      (0x13000002|SPI020_CE_VALUE|SPI020_INTR_CFG)
/*******************************************
 * for 4Bytes dual read data command for windond device (0x3C)
 ********************************************/
/* Set command word 0, set SPI flash address by input parameter */
...
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 8, dum_1st_cyc = 0, address length = 3 */
#define SPI020_3C_CMD1    0x01180002
/* Set command word 2, set data count by input parameter */
#define SPI020_3C_CMD2      0x0
/* Set command word 4, instrction code = 0x3B, contiune read = 0,
   start_ce = ??, spi mode = 1, DTR mode = 0, status = 0,
...
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 8, dum_1st_cyc = 0, address length = 4 */
#define SPI020_6C_CMD1      0x01180002
/* Set command word 2, set data count by input parameter */
#define SPI020_6C_CMD2      0x0
/* Set command word 4, instrction code = 0x6C, contiune read = 0,
   start_ce = ??, spi mode = 2, DTR mode = 0, status = 0,
...
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 4, dum_1st_cyc = 0, address length = 4 */
#define SPI020_BC_CMD1      0x010C0002
/* Set command word 2, set data count by input parameter */
#define SPI020_BC_CMD2      0x0
/* Set command word 4, instrction code = 0xBC, contiune read = 0,
   start_ce = ??, spi mode = 3, DTR mode = 0, status = 0,
...
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 6, dum_1st_cyc = 0, address length = 4 */
#define SPI020_EC_CMD1    0x010A0002
/* Set command word 2, set data count by input parameter */
#define SPI020_EC_CMD2      0x0
/* Set command word 4, instrction code = 0xEB, contiune read = 0,
   start_ce = ??, spi mode = 4, DTR mode = 0, status = 0,
...
#define SPI020_EC_CMD3      (0xEC000080|SPI020_CE_VALUE|SPI020_INTR_CFG)
/*******************************************
 * for Read BBM Look Up Table command (0xA5)
 ********************************************/
/* Set command word 0, set SPI flash address by input parameter */
#define SPI020_A5_CMD0      0x0
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 0, dum_1st_cyc = 0, address length = 3 */
#define SPI020_A5_CMD1      0x01000000  //bessel:change value from 0x01000003 to 0x01080003(Fast Read instruction need to add eight "dummy"clocks after 24-bit address)
/* Set command word 2, set data count by input parameter */
#define SPI020_A5_CMD2      0
/* Set command word 4, instrction code = 0x03, contiune read = 0,
   start_ce = ??, spi mode = 0, DTR mode = 0, status = 0,
   status_en = 0, write enable = 0, intr_en = ? */
#define SPI020_A5_CMD3      (0xA5000000|SPI020_CE_VALUE|SPI020_INTR_CFG)
/*******************************************
...
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 0, dum_1st_cyc = 0, address length = 3 */
#define SPI020_02_CMD1      0x01000002
/* Set command word 2, set data count by input parameter */
...
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 0, dum_1st_cyc = 0, address length = 3 */
#define SPI020_32_CMD1      0x01000002
/* Set command word 2, set data count by input parameter */
...
/*******************************************
 * for Page Execute command (0x10)
 ********************************************/
/* Set command word 0, set page address by input parameter */
#define SPI020_10_CMD0      0
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 0, dum_1st_cyc = 0, address length = 4 */
#define SPI020_10_CMD1      0x01000003
/* Set command word 2, set data count by input parameter */
#define SPI020_10_CMD2      0x0
/* Set command word 4, instrction code = 0x13, contiune read = 0,
   start_ce = ??, spi mode = 0, DTR mode = 0, status = 0,
   status_en = 0, write enable = 0, intr_en = ? */
#define SPI020_10_CMD3      (0x10000002|SPI020_CE_VALUE|SPI020_INTR_CFG)
...
/*******************************************
 * for block erase command (0xD8):128KB Block Erase
 ********************************************/
/* Set command word 0, set SPI flash address by input parameter */
...
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 8, dum_1st_cyc = 0, address length = 3 */
#define SPI020_6B_CMD1    0x01080002 /* F 0x01080003 */ /* E 0x01080002 */
/* Set command word 2, set data count by input parameter */
 ...
 ********************************************/
/* Set command word 0, set SPI flash address by input parameter */
#define SPI020_6B_CMD0_f      0x0
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 8, dum_1st_cyc = 0, address length = 3 */
#define SPI020_6B_CMD1_f      0x01080003 /* F 0x01080003 */ /* E 0x01080002 */
/* Set command word 2, set data count by input parameter */
#define SPI020_6B_CMD2_f      0x0
/* Set command word 4, instrction code = 0x3B, contiune read = 0,
   start_ce = ??, spi mode = 2, DTR mode = 0, status = 0,
   status_en = 0, write enable = 0, intr_en = ? */
#define SPI020_6B_CMD3_f      (0x6B000040|SPI020_CE_VALUE|SPI020_INTR_CFG)
...
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 6, dum_1st_cyc = 0, address length = 3 */
#define SPI020_EB_CMD1    0x01020002 /* 1G dummy 2bits 2/4G dummy 8bits */
#define SPI020_EB_CMD1_5E    0x01040002 /* gd5E 1G dummy 4bits */
/* Set command word 2, set data count by input parameter */
...
/* Set command word 4, instrction code = 0x01, contiune read = 0,
   start_ce = ??, spi mode = 0, DTR mode = 0, status = 0,
   status_en = 0, write enable = 1, intr_en = ? */
#define SPI020_01_CMD3      (0x1F000002|SPI020_CE_VALUE|SPI020_INTR_CFG)
/*******************************************
 * for WRSR (write-status-register) command (0x01)
 ********************************************/
/* Set command word 0, set SPI flash address to 0 */
#define SPI020_01_CMD0_w    0x0
/* Set command word 1, continue read = 0, instruction leng = 1,
   dum_2nd_cyc = 0, dum_1st_cyc = 0, address length = 0 */
#define SPI020_01_CMD1_w    0x01000001
/* Set command word 2, set data count to 1 */
#define SPI020_01_CMD2_w    0x1
/* Set command word 4, instrction code = 0x01, contiune read = 0,
   start_ce = ??, spi mode = 0, DTR mode = 0, status = 0,
   status_en = 0, write enable = 1, intr_en = ? */
#define SPI020_01_CMD3_w    (0x1F000002|SPI020_CE_VALUE|SPI020_INTR_CFG)
/*******************************************
...
// reset device
#define SPI020_FF_CMD0      0x0
#define SPI020_FF_CMD1      0x01000000
#define SPI020_FF_CMD2      0x00
#define SPI020_FF_CMD3      (0xFF000002|SPI020_CE_VALUE|SPI020_INTR_CFG)
