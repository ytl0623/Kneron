...
IMAGE_FILE_PATH = os.path.join(PWD, '../../res/images/people_cars_street_1500x1500.bmp')
LOOP_TIME = 50

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='KL720 Demo Generic Inference with Crop-Box Example.')
    ...
    try:
        ...
        for idx, generic_crop_raw_result in enumerate(generic_crop_raw_result_list):
            yolo_result = post_process_yolo_v5(
                ...
                hardware_preproc_info=generic_crop_raw_result.header.hw_pre_proc_info,
                thresh_value=0.15,
                with_sigmoid=False)
    ...
    for idx, yolo_result in enumerate(app_yolo_result_list):
        print('\n[Crop Box {}]'.format(idx))
        print(' - [Crop Box Information]\n{}'.format(generic_raw_image_header.inference_crop_box_list[idx]))
    ...
    for idx, generic_crop_raw_result in enumerate(generic_crop_raw_result_list):
        ...
        inference_crop_box = generic_crop_raw_result.header.hw_pre_proc_info.crop_area
