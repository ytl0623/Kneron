...
def _image_send_function(_device_group: kp.DeviceGroup,
                         ...
                         _image_format: kp.ImageFormat,
                         _image_width: Union[int, None] = None,
                         _image_height: Union[int, None] = None) -> None:
    for _loop in range(_loop_time):
        try:
            _generic_raw_image_header.inference_number = _loop
...
def _result_receive_function(_device_group: kp.DeviceGroup,
                             _loop_time: int,
                             _generic_raw_image_header: kp.v1.GenericRawImageHeader,
                             _model_nef_descriptor: kp.ModelNefDescriptor,
                             _result_queue: queue.Queue) -> None:
    _generic_raw_result = None

    for _loop in range(_loop_time):
        try:
            _generic_raw_result = kp.v1.inference.generic_raw_inference_receive(device_group=_device_group,
                                                                                generic_raw_image_header=_generic_raw_image_header,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='KL520 Demo Generic Inference Multi-Thread Example.')
