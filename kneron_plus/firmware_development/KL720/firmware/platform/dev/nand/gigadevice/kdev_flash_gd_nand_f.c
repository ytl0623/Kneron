...
*  Description:
*  ------------
*  This SPI Flash driver is specific for GigaDevice GD5F1GQ4xFxIG SPI Flash Access
*  HW: Faraday FTSPI020
*
******************************************************************************/
#include "cmsis_os2.h"
...
#if defined(FLASH_TYPE) && (FLASH_TYPE == FLASH_TYPE_GIGADEVICE_NAND)
//#define FLASH_GD_DBG
#ifdef FLASH_GD_DBG
#define flash_msg(fmt, ...) kmdw_printf("[GIGADEVICE_FLASH] " fmt, ##__VA_ARGS__)
#else
...
uint8_t kdev_flash_read_BBM(uint32_t block_index)
{
    ...
    kdrv_spif_set_commands(0x800, SPI020_03_CMD1_f, read_lens, SPI020_03_CMD3_f);
...
void kdev_flash_read_status(void)
{
    ...
    uint32_t RDCR=0; //B0h
    uint32_t RDFeature=0; //D0h
    uint32_t RDStatus=0; //F0h
    ...
    kdrv_spif_set_commands( ADR_STATUS_REG_4 , SPI020_05_CMD1_w, SPI020_05_CMD2_w, SPI020_05_CMD3_w );
    nrx_buff_word_index = 0;
    kdrv_spif_read_Rx_FIFO( &RDFeature, &nrx_buff_word_index, 0x01 );
    flash_msg("Feature D0h = 0x%2X\n", (uint8_t)RDFeature );
    kdrv_spif_wait_command_complete();

    kdrv_spif_set_commands( ADR_STATUS_REG_5 , SPI020_05_CMD1_w, SPI020_05_CMD2_w, SPI020_05_CMD3_w );
    nrx_buff_word_index = 0;
    kdrv_spif_read_Rx_FIFO( &RDStatus, &nrx_buff_word_index, 0x01 );
    flash_msg("Status F0h = 0x%2X\n", (uint8_t)RDStatus );
    kdrv_spif_wait_command_complete();
...
uint32_t kdev_flash_LUT_SWAP(uint32_t address)
{
    ...
    kdrv_spif_set_commands(page_src<<1, SPI020_03_CMD1_f, read_lens, SPI020_03_CMD3_f);
    kdrv_spif_wait_rx_full();
    readdata = regSPIF_data->dw.kdrv_spif_dp;
...
void kdev_flash_pageread(uint8_t  type, uint32_t  offset, uint32_t  len, void *buf)
{
    ...
    if(type & FLASH_QUAD_RW)
    {
        if(type & FLASH_IO_RW) /* Quad IO */
            ...
            kdrv_spif_set_commands(0, SPI020_6B_CMD1_f, len, SPI020_6B_CMD3_f);
    }
    else
    {
        kdrv_spif_set_commands(0, SPI020_03_CMD1_f, len, SPI020_03_CMD3_f);
    }
...
void kdev_flash_read(uint8_t  type, uint32_t  offset, uint32_t  len, void *buf)
{
    ...
    if (type & FLASH_DTR_RW) {
        ...
        if(type & FLASH_IO_RW) {
            ...
            kdrv_spif_set_commands(offset, SPI020_6B_CMD1_f, len, SPI020_6B_CMD3_f);
        }
        ...
        kdrv_spif_set_commands(offset, SPI020_03_CMD1_f, len, SPI020_03_CMD3_f);
    }
