...
*  Description:
*  ------------
*  This SPI Flash driver is specific for Winbond W25N01GVxxxG SPI Flash Access
*  HW: Faraday FTSPI020
*
******************************************************************************/
#include "cmsis_os2.h"
...
//#define FLASH_WB_DBG
#ifdef FLASH_WB_DBG
#define flash_msg(fmt, ...) kmdw_level_printf(LOG_CUSTOM, "[WINBOND_FLASH] " fmt, ##__VA_ARGS__)
#else
...
kdev_status_t kdev_flash_check_cumulativeECCstauts(void)
{
    ...
    if(status & SR3_P_FAIL) {
        ...
        kmdw_printf("LOG_ERROR: kdev_flash_check_cumulativeECCstauts SR3_P_FAIL 0x%X!\n",status);
    }
    if(status & SR3_E_FAIL) {
        ret |= KDEV_STATUS_EFAIL;
        kmdw_printf("LOG_ERROR: kdev_flash_check_cumulativeECCstauts SR3_E_FAIL 0x%X!\n",status);
    }
    if(status&SR3_ECC_1) {
        ret |= KDEV_STATUS_ECC_ERROR;
        kmdw_printf("LOG_ERROR: kdev_flash_check_cumulativeECCstauts SR3_ECC_1 0x%X!\n",status);
    }
    else if(status&SR3_ECC_CHECK) {
        ret |= KDEV_STATUS_ECC_CORRECT;
        kmdw_printf("LOG_ERROR: kdev_flash_check_cumulativeECCstauts SR3_ECC_CHECK 0x%X!\n",status);
    }
#endif
    return ret;
}
...
uint8_t kdev_flash_read_BBM(uint32_t block_index)
{
    ...
    //flash_msg("Spare byte 0 in block %d= 0x%0X, %d\n", block_index, readdata, read_lens );
...
#ifdef SPIF_READ_DMA_EN
void kdev_flash_pageread_dma(uint8_t  type, uint32_t  offset, uint32_t  len, void *buf, gdma_xfer_callback_t xfer_isr_cb, void *usr_arg)
{
    uint32_t  *read_buf;
    ...
    {
        if(type & FLASH_IO_RW) /* Quad IO */
            kdrv_spif_set_commands(0, SPI020_BC_CMD1, len, SPI020_BC_CMD3);
        else /* Quad Output */
            kdrv_spif_set_commands(0, SPI020_3C_CMD1, len, SPI020_3C_CMD3);
    }
    else
    {
    ...
    kdrv_gdma_transfer(ch,(uint32_t)read_buf, (uint32_t)&regSPIF_data->dw.kdrv_spif_dp, (uint32_t)len, xfer_isr_cb, usr_arg);

    regSPIF_irq->st.bf.kdrv_spif_icr.cmd_cmplt_intr_en = 1;
    kdrv_spif_check_status_till_ready();/* wait for command complete */ //kdrv_spif_wait_command_complete();
...
kdev_status_t kdev_flash_continuous_read(uint32_t addr, void *data, uint32_t cnt) 
{
    ...
    uint32_t read_data;
    uint32_t rx_fifo_depth = kdrv_spif_rxfifo_depth();
    uint32_t total_length = cnt;
    uint32_t *read_buf = (uint32_t  *)data;
    uint32_t tmp = 0;

    if(total_length > SPI020_BLOCK_128SIZE)
    {
        kmdw_printf("read size must within 1 block");
        return KDEV_STATUS_ERROR;
    }
    ...
    kdrv_spif_set_commands(ADR_STATUS_REG_2, SPI020_01_CMD1_w, SPI020_01_CMD2_w, SPI020_01_CMD3_w );
    RDCR &= ~BIT3;/* Continuous Read Mode */
    kdrv_spif_write_data((uint8_t *)&RDCR, 1);
    kdrv_spif_wait_command_complete();//spi020_check_status_til_ready();
    //flash_msg("Continuous Read Mode 0x%X !\n",(uint8_t)RDCR);

    //continuous read mode
    kdrv_spif_set_commands((address / spi_nand_data_buf_size), SPI020_13_CMD1, SPI020_13_CMD2, SPI020_13_CMD3);
    //kdrv_delay_us(60);
    kdrv_spif_check_status_till_ready();
    ...
    {
        if(FLASH_OP_MODE & FLASH_IO_RW) /* Quad IO */
            kdrv_spif_set_commands(0, SPI020_EC_CMD1, total_length, SPI020_EC_CMD3);
        else /* Quad Output */
            kdrv_spif_set_commands(0, SPI020_6C_CMD1, total_length, SPI020_6C_CMD3);
    }
    else if(FLASH_OP_MODE & FLASH_DUAL_READ)
    {
        if(FLASH_OP_MODE & FLASH_IO_RW) /* Quad IO */
            kdrv_spif_set_commands(0, SPI020_BC_CMD1, total_length, SPI020_BC_CMD3);
        else /* Quad Output */
            kdrv_spif_set_commands(0, SPI020_3C_CMD1, total_length, SPI020_3C_CMD3);
    }
    else
    {
        kdrv_spif_set_commands(0, SPI020_03_CMD1, total_length, SPI020_03_CMD3);
    }
    ...
    while (total_length > 0)
    {
        ...
        access_byte = min_t(total_length, rx_fifo_depth);
        total_length -= access_byte;
    ...
    kdrv_spif_set_commands(ADR_STATUS_REG_2, SPI020_01_CMD1_w, SPI020_01_CMD2_w, SPI020_01_CMD3_w );
    RDCR |= BIT3;/* BUFFER Read Mode */
    kdrv_spif_write_data((uint8_t *)&RDCR, 1);
    kdrv_spif_wait_command_complete();//spi020_check_status_til_ready();
    //flash_msg("BUFFER Read Mode 0x%X !\n",(uint8_t)RDCR);
    return KDEV_STATUS_OK;
}

#ifdef SPIF_CONTINUOUS_READ_DMA_EN
kdev_status_t kdev_flash_continuous_read_dma(uint32_t addr, void *data, uint32_t cnt, gdma_xfer_callback_t xfer_isr_cb, void *usr_arg) /* there's no BBM handling, must be careful */
{
    uint32_t RDCR=0;
    uint32_t rx_fifo_depth = kdrv_spif_rxfifo_depth();
    uint32_t total_length = cnt;
    ...
    dma_settings.src_addr_ctrl = GDMA_FIXED_ADDRESS;
    kdrv_gdma_configure_setting(ch, (gdma_setting_t *) &dma_settings);
    kdrv_gdma_transfer(ch,(uint32_t)read_buf, (uint32_t)&regSPIF_data->dw.kdrv_spif_dp, (uint32_t)total_length, xfer_isr_cb, usr_arg);
...
kdev_status_t _kdev_flash_continuous_read_all(uint32_t address, void *data, uint32_t cnt, bool dma, gdma_xfer_callback_t xfer_isr_cb, void *usr_arg)
{
    uint32_t rx_fifo_depth = kdrv_spif_rxfifo_depth();
    uint32_t total_length = cnt;
    uint32_t *read_buf = (uint32_t  *)data;
...
{
    uint32_t logical_blk_src = src / SPI020_BLOCK_128SIZE;
    uint32_t logical_blk_end = (src + cnt) / SPI020_BLOCK_128SIZE;
    uint32_t page_start = src-(logical_blk_src*SPI020_BLOCK_128SIZE);
    uint32_t physical_blk_start = 0;
    uint32_t physical_blk_next = 0;
    uint32_t physical_blk_last = 0;
    uint32_t len = cnt;
    uint32_t i = 0;
    uint32_t loop = 0;
    uint32_t RDCR=0;

    //printf("kdev_flash_continuous_read_all %x, %x, %d\n",src,dst,cnt);
    kdrv_spif_set_commands(ADR_STATUS_REG_2, SPI020_05_CMD1_w, SPI020_05_CMD2_w, SPI020_05_CMD3_w );
    uint16_t nrx_buff_word_index = 0;
    kdrv_spif_read_Rx_FIFO( &RDCR, &nrx_buff_word_index, 0x01 );
    //flash_msg("Configuration B0h = 0x%2X\n", (uint8_t)RDCR );
    ...
    if((src + cnt) % SPI020_BLOCK_128SIZE) logical_blk_end++;

    do {
        physical_blk_start = bbm_lut[logical_blk_src];
        loop = 1;
        
        //check +1
        physical_blk_last = physical_blk_start;
        do
        {
            logical_blk_src++;
            physical_blk_next = bbm_lut[logical_blk_src];
            if((physical_blk_next - physical_blk_last) > 1)
                break;
            physical_blk_last = physical_blk_next;
            loop++;
        } while(logical_blk_src < logical_blk_end);
        uint32_t size = (len > (loop*SPI020_BLOCK_128SIZE)) ? (loop*SPI020_BLOCK_128SIZE) : len;
        len -= size;
        #ifdef SPIF_CONTINUOUS_READ_DMA_EN
            #define DMA_LIMIT_BLK 127
            #define DMA_LIMIT (DMA_LIMIT_BLK*SPI020_BLOCK_128SIZE)
            uint32_t left_sz=size;
            uint32_t loop_dma = loop / DMA_LIMIT_BLK;
            if(loop % DMA_LIMIT_BLK) loop_dma++;

            for(int j=0; j<loop_dma; j++)
            {
                uint32_t sz = (left_sz > DMA_LIMIT) ? DMA_LIMIT : left_sz;
                left_sz -= sz;
                _kdev_flash_continuous_read_all(((physical_blk_start*SPI020_BLOCK_128SIZE)+(j*DMA_LIMIT))+page_start, (void*)(dst+(i*SPI020_BLOCK_128SIZE)+(j*DMA_LIMIT)), sz, true, xfer_isr_cb, usr_arg);
            }
        #else
            _kdev_flash_continuous_read_all((physical_blk_start*SPI020_BLOCK_128SIZE)+page_start, (void*)(dst + (i * SPI020_BLOCK_128SIZE)), size, false, NULL, NULL);
        #endif
        i += loop;
    } while(logical_blk_src < logical_blk_end);
