...
#define TEST_MESSAGE(message)                   dbg_msg(__FILE__, __LINE__, message)
#define TEST_ASSERT_MESSAGE(condition,message)  if (condition) { dbg_msg (__FILE__, __LINE__, "SUCCESS"); } else dbg_msg (__FILE__, __LINE__, message, "FAILED")
*/
/**
...
*/
/**
  \fn            int32_t ARM_WIFI_SocketConnect (int32_t socket, const uint8_t *ip, uint32_t ip_len, uint16_t port,uint32_t mode)
  \brief         Connect a socket to a remote host.
  \param[in]     socket   Socket identification number
  \param[in]     ip       Pointer to remote IP address
  \param[in]     ip_len   Length of 'ip' address in bytes
  \param[in]     port     Remote port number
	\param[in]		 mode 		TCP mode[0: normal tcp connect 1:SSL connection]
  \return        status information
                   - 0                                 : Operation successful
                   - \ref ARM_SOCKET_ESOCK             : Invalid socket
                   - \ref ARM_SOCKET_EINVAL            : Invalid argument
...
typedef struct {
  ...
  int32_t               (*SocketConnect)               (int32_t socket, const uint8_t *ip, uint32_t  ip_len, uint16_t  port,uint32_t mode);
