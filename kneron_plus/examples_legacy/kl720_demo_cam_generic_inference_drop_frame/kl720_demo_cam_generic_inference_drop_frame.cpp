/**
 * @file        kl720_demo_cam_user_define_api_drop_frame.cpp
...
void *image_send_function(void *data)
{
    ...
    _input_desc.width = _image_width;                  // image width
    _input_desc.height = _image_height;                // image height
    ...
    while (true)
    {
        ...
        int ret = kp_generic_raw_inference_send(_device, &_input_desc, (uint8_t *)_cv_img_rgb565.data);
        if (ret != KP_SUCCESS)
        {
...
{
    // tiny yolo v5 outputs 3 nodes, described by _output_desc.num_output_node
    ...
    {
        /* Receive one result of yolo inference */
        int ret = kp_generic_raw_inference_receive(_device, &_output_desc, raw_output_buf, raw_buf_size);
        if (ret != KP_SUCCESS)
        {
        ...
        post_process_yolo_v5_720(output_nodes, _output_desc.num_output_node, &_output_desc.pre_proc_info, 0.15, &_yolo_result_latest);
        _mutex_result.unlock();
