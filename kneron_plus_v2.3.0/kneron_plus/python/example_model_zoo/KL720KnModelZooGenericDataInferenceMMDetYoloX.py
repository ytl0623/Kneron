...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='KL720 Kneron Model Zoo Generic Data Inference Example. - YoloX.')
    ...
    parser.add_argument('-img',
                        '--img_path',
                        default=os.path.join(PWD, '../../res/images/people_talk_in_street_640x640.bmp'),
                        type=str)
    parser.add_argument('-nef',
                        '--nef_model_path',
                        help='input NEF model path',
                        default=os.path.join(PWD,
                                             '../../res/models/KL720/kn-model-zoo-mmdet_yolox/example_yolox_720.nef'),
                        type=str)
    ...
    kwargs = {'input_shape': [nef_model_width, nef_model_height], 'num_classes': 1, 'conf_threshold': 0.1,
              'iou_threshold': 0.5, 'top_k_num': 300}
    ...
    for bbox in det_res:
        ...
        print("(" + str(int(xmin)) + "," + str(int(ymin)) + ',' + str(int(xmax)) + ',' + str(int(ymax)) + ")")
