...
static void *_update_kdp2_firmware_to_single_device(void *data)
{
    ...
    if (KP_DEVICE_KL520 == ll_dev->dev_descp.product_id) {
        ...
        ret = kp_write_data_to_flash(ll_dev, cmd_pack->timeout, flash_offset, cmd_pack->fw_size, (uint8_t *)cmd_pack->fw_buf);
                ...
                (KP_DEVICE_KL730 == ll_dev->dev_descp.product_id) ||
                (KP_DEVICE_KL830 == ll_dev->dev_descp.product_id)) {
        /** update firmware */
    ...
    if (true == cmd_pack->auto_reboot)
    {
        ...
        if ((KP_DEVICE_KL730 == ll_dev->dev_descp.product_id) ||
            (KP_DEVICE_KL830 == ll_dev->dev_descp.product_id)) {
            ...
            ret = kp_usb_write_data(ll_dev, (void *)&cmd_buf, sizeof(cmd_buf), cmd_pack->timeout);
        }
    }
...
static void *_update_kdp2_loader_to_single_device(void *data)
{
    ...
    if (true == cmd_pack->auto_reboot)
    {
        ...
        if ((ret != KP_USB_USB_PIPE) &&
            ...
            (ret != KP_SUCCESS)) {// FIXME: tricky way
                                  // Note that device return KP_SUCCESS in some Windows device and KP_USB_USB_NO_DEVICE in Ubuntu) // FIXEME: trikcy way
            cmd_pack->sts = KP_ERROR_RESET_FAILED_25;
            return NULL;
        }
...
int _kl630_update_kdp2_usb_loader(kp_device_group_t devices, void *usb_loader_buf, int usb_loader_size, bool auto_reboot)
{
    ...
    for (int i = 0; i < _devices_grp->num_device; i++) {
        ...
        uint16_t fw_type_legacy = (KP_KDP2_FW_FIND_TYPE_MASK & ll_dev[i]->fw_serial);
            dbg_print("fw_type = 0x%x \n", fw_type);
            dbg_print("fw_type_legacy = 0x%x \n", fw_type_legacy);
            dbg_print("fw_serial = 0x%x \n", ll_dev[i]->fw_serial);

        if ((KP_KDP2_FW_LOADER_V2 == fw_type) || (KP_KDP2_FW_LOADER == fw_type_legacy) || (KP_KDP_FW == ll_dev[i]->fw_serial)) {
            dbg_print("%s one or more devices are running KDP or KDP2 Loader...\n", __FUNCTION__);
            dbg_print("fw_type = 0x%x \n", fw_type);
            dbg_print("fw_type_legacy = 0x%x \n", fw_type_legacy);
            dbg_print("fw_serial = 0x%x \n", ll_dev[i]->fw_serial);
                  ...
                  kp_model_nef_descriptor_t *model_desc)
{
    ...
    for (int i = 0; i < _devices_grp->num_device; i++) {
        if (((KP_DEVICE_KL520 == ll_dev[i]->dev_descp.product_id) && (KP_KDP2_FW_LOADER_V2 == (KP_KDP2_FW_FIND_TYPE_MASK_V2 & ll_dev[i]->fw_serial))) ||
            ...
            dbg_print("%s, one or more devices are running KDP2 Loader...\n", __FUNCTION__);
            return KP_ERROR_INVALID_FIRMWARE_24;
        } else if ((KP_DEVICE_KL630 == ll_dev[i]->dev_descp.product_id) ||
                   (KP_DEVICE_KL730 == ll_dev[i]->dev_descp.product_id) ||
                   (KP_DEVICE_KL830 == ll_dev[i]->dev_descp.product_id)) {
            dbg_print("KL630/KL730/KL830 does not support _update_model.\n");
            return KP_ERROR_INVALID_FIRMWARE_24;
        }
    }
                ...
                kp_model_nef_descriptor_t *model_desc)
{
    ...
    for (int i = 0; i < _devices_grp->num_device; i++) {
        if ((KP_DEVICE_KL630 == ll_dev[i]->dev_descp.product_id) &&
            (KP_KDP2_FW_LOADER_V2 == (KP_KDP2_FW_FIND_TYPE_MASK_V2 & ll_dev[i]->fw_serial))) {
            dbg_print("one or more devices are running KDP2 Loader...\n");
            return KP_ERROR_INVALID_FIRMWARE_24;
        } else if ((KP_DEVICE_KL730 == ll_dev[i]->dev_descp.product_id) &&
                   (KP_KDP2_FW_LOADER_V2 == (KP_KDP2_FW_FIND_TYPE_MASK_V2 & ll_dev[i]->fw_serial))) {
            dbg_print("%s, one or more devices are running KDP2 Loader...\n", __FUNCTION__);
            return KP_ERROR_INVALID_FIRMWARE_24;
        } else if ((KP_DEVICE_KL830 == ll_dev[i]->dev_descp.product_id) &&
                   (KP_KDP2_FW_LOADER_V2 == (KP_KDP2_FW_FIND_TYPE_MASK_V2 & ll_dev[i]->fw_serial))) {
            dbg_print("%s, one or more devices are running KDP2 Loader...\n", __FUNCTION__);
            return KP_ERROR_INVALID_FIRMWARE_24;
            ...
            dbg_print("KL520 and KL720 do not support _update_nef.\n");
            return KP_ERROR_INVALID_FIRMWARE_24;
        }
    }
                           ...
                           void *ncpu_fw_buf, int ncpu_fw_size, bool auto_reboot)
{
    ...
    for (int i = 0; i < _devices_grp->num_device; i++) {
        if (((KP_DEVICE_KL520 == ll_dev[i]->dev_descp.product_id) && (KP_KDP2_FW_LOADER_V2 == (KP_KDP2_FW_FIND_TYPE_MASK_V2 & ll_dev[i]->fw_serial))) ||
            ...
            ((KP_DEVICE_KL730 == ll_dev[i]->dev_descp.product_id) && (KP_KDP2_FW_LOADER_V2 == (KP_KDP2_FW_FIND_TYPE_MASK_V2 & ll_dev[i]->fw_serial))) ||
            ((KP_DEVICE_KL830 == ll_dev[i]->dev_descp.product_id) && (KP_KDP2_FW_LOADER_V2 == (KP_KDP2_FW_FIND_TYPE_MASK_V2 & ll_dev[i]->fw_serial)))) {
            dbg_print("%s, one or more devices are running KDP2 Loader...\n", __FUNCTION__);
            return KP_ERROR_INVALID_FIRMWARE_24;
        }
    }
                    ...
                    kp_model_nef_descriptor_t *model_desc)
{
    ...
    if ((KP_DEVICE_KL630 == ll_dev[0]->dev_descp.product_id) ||
        (KP_DEVICE_KL730 == ll_dev[0]->dev_descp.product_id) ||
        (KP_DEVICE_KL830 == ll_dev[0]->dev_descp.product_id)) {
    ...
    for (int i = 0; i < _devices_grp->num_device; i++) {
        if (0 == i) {
            ...
            if ((KP_DEVICE_KL520 != product_id) &&
                ...
                (KP_DEVICE_KL730 != product_id) &&
                (KP_DEVICE_KL830 != product_id)) {
                dbg_print("product_id = %d ,one or more devices not support KDP2 Loader update...\n", product_id);
                ret = KP_ERROR_UNSUPPORTED_DEVICE_44;
                goto FUNC_OUT;
            }
