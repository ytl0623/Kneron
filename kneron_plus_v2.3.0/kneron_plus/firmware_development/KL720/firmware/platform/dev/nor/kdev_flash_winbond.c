...
#if defined(FLASH_TYPE) && (FLASH_TYPE == FLASH_TYPE_WINBOND_NOR)
//#define FLASH_WB_DBG
#ifdef FLASH_WB_DBG
#define flash_msg(fmt, ...) info_msg("[WINBOND_FLASH] " fmt, ##__VA_ARGS__)
#else
...
void kdev_spif_dma_enable(void)
{
    regSPIF_irq->st.bf.kdrv_spif_icr.DMA_EN = 1;/* enable DMA function */
    regSPIF_irq->st.bf.kdrv_spif_icr.cmd_cmplt_intr_en = 1;/* enable DMA function */
}

void kdev_spif_dma_read_stop(void)
{
    kdrv_spif_wait_command_complete();/* wait for command complete */
    regSPIF_irq->st.bf.kdrv_spif_icr.DMA_EN = 0;/* disable DMA function */
    regSPIF_irq->st.bf.kdrv_spif_icr.cmd_cmplt_intr_en = 1;/* disable DMA function */
}

void kdrv_spif_flash_to_ddr_dma(uint32_t *dst, uint32_t bytes)
{
    kdev_spif_dma_enable();

    kdrv_spif_read_data_dma(dst, bytes);

    kdev_spif_dma_read_stop();
}
#endif
...
#ifndef SPIF_GDMA_ENABLE
    kdrv_spif_read_data(read_buf, len);/* read data */
    kdrv_spif_wait_command_complete();/* wait for command complete */
#else
    kdrv_spif_flash_to_ddr_dma(read_buf, len);
#endif
...
#ifdef SPIF_GDMA_ENABLE
    kdrv_spif_acquire_dma();
    flash_msg("=========== ACQUIRE DMA ===========\n");
#endif
    Option = FLASH_OP_MODE;
    kdev_flash_read(Option, addr , cnt , data);
#ifdef SPIF_GDMA_ENABLE
    kdrv_spif_release_dma();
    flash_msg("=========== RELEASE DMA ===========\n");
#endif
    return KDEV_STATUS_OK;
}
...
kdev_status_t kdev_flash_programdata_memxfer(uint32_t addr, const void *data, uint32_t cnt)
{
    ...
    {
        kmdw_printf("LOG_CRITICAL: address and size must align to 4K bytes!!\n");
