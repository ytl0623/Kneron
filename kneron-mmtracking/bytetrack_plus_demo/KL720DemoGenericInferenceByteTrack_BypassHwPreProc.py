...
from utils.bytetrack_postprocess import postprocess_ as bytetrack_postproc
from utils.bytetrack_postprocess import BYTETracker
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='KL720 Kneron Model Zoo Generic Inference Bypass Pre-Processing Example. - Bytetrack. ')
    ...
    """
    tmp_array = np.zeros((nef_model_height, nef_model_width, 3))
    tmp_array = convert_numpy_to_rgba_and_width_align_4(tmp_array)
    generic_raw_image_header = kp.GenericRawBypassPreProcImageHeader(
        model_id=model_nef_descriptor.models[0].id,
        image_buffer_size=len(tmp_array),
        inference_number=0
    )
    ...
    while(cap.isOpened()):
        ...
        kwargs = {'input_shape': [nef_model_height, nef_model_width], 'num_classes': 1, 'conf_threshold': 0.1, 'iou_threshold': 0.7, 'top_k_num': 300}
        ...
        for idx, bbox in enumerate(track_res):
            ...
            id = person_ids[idx]
            cv2.putText(img, str(id), (int(xmin), int(ymin)), cv2.FONT_HERSHEY_SIMPLEX, 1, (125, 200, 200), 5, cv2.LINE_AA)
            cv2.rectangle(img, (int(xmin), int(ymin)), (int(xmax), int(ymax)), (0, 255, 0), 2)
            cv2.putText(img, "Press 'Q' key to quit", (int(20), int(40)), cv2.FONT_HERSHEY_SIMPLEX, 1, (105, 100, 250), 2, cv2.LINE_AA)
